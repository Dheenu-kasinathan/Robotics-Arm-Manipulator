...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        for link_name in link_names:
            # get current link number/name
            #print "This is %s" % link_name
            link_idx = link_names.index(link_name)
            #print "link index %d" % link_idx
            
            #get transform of the previous joint
            if link_idx == 0:
                tfp = T
            else:
                tfp = linktf[link_idx]
            
            # get current link message and compute transform to the end of this link
            jtc = joints[link_idx]
            jtxyz = jtc.origin.xyz
            jtrpy = jtc.origin.rpy
            jtname = jtc.name
            tftemp = numpy.dot(tf.transformations.translation_matrix(jtxyz),tf.transformations.translation_matrix(jtrpy))
       
            if jtc.type == 'fixed':
                # fixed joint
                #print "Fixed joint"
                tfc = tftemp
            elif jtc.type == 'revolute':
                #print "Revolute joint"
                jt_rotaxis = jtc.axis
                # get the current joint value and rotate joint angle
                jt_idx = joint_values.name.index(jtname)
                jt_val = joint_values.position[jt_idx]
                #print "jt_idx = %d" %jt_idx
                #print "jt_val = %d" %jt_val
                #print "jt_rotaxis = %s" %jt_rotaxis
                q_rot = tf.transformations.quaternion_about_axis(jt_val, jt_rotaxis)
                jtrot = tf.transformations.quaternion_matrix(q_rot)
                tfc = numpy.dot(tftemp,jtrot)
        
                # print "jt name = %s" %jtname
            else:
                #print "wrong rotation axis"
                tfc = tftemp
        
            basetf = numpy.dot(tfp,tfc)
            linktf.append(basetf)
            Tc = convert_to_message(basetf,link_name,"world_link")
        
            all_transforms.transforms.append(Tc)
